	...
	map.setCenter(
		lonLatToMercator(
			new OpenLayers.LonLat(
				${startParameters.geoLocation.longitude},
				${startParameters.geoLocation.latitude}
			)),
		${startParameters.zoom});
...
function updateAmenities(event, url, forceUpdate) {
	...
	if (zoom > MIN_ZOOM_FOR_EDIT) {
		...
		|| url != URL.search) {
			...
			params.north = north, params.south = south, params.west = west, params.east = east;
...
function createKeyValueTable(amenity) {
	...
	var formTag = new Element("form", {
			"id": "form_" + nodeId, "action": URL.amenity, "method": "post"
		});
	formTag.insert(new Element("input", {
			"type": "hidden", "name": "_nodeId", "value": nodeId
		}));
	formTag.insert(new Element("input", {
			"type": "hidden", "name": "lon", "value": amenity.lon
		}));
	formTag.insert(new Element("input", {
			"type": "hidden", "name": "lat", "value": amenity.lat
		}));
	if (amenity.newLon && amenity.newLat) {
		formTag.insert(new Element("input", {
				"type": "hidden", "name": "newlon", "value": amenity.newLon
			}));
		formTag.insert(new Element("input", {
				"type": "hidden", "name": "newlat", "value": amenity.newLat
			}));
	}
	formTag.insert(new Element("input", {
			"type": "hidden", "name": "_method", "value": nodeId > 0 ? "put" : "post"
		}));
...
function createTagValue(nodeId, key, value) {
		...
		newDiv.insert(new Element("input", {
				"type": "text", "id": keyId, "name": "key", "class": "inputkey", "size": 24, "value": key
			}));
		newDiv.insert(new Element("div", {
				"id": keyIdChoices, "class": "autocomplete"
			}));
		newDiv.insert(new Element("script").update(
			"new Ajax.Autocompleter('" + keyId + "', '" + keyId + "_choices', '" + URL.acKey +
			"', {'paramName': 'input', 'method': 'get', 'minChars': 1, 'frequency': 0.5});"
		));
		newDiv.insert(new Element("span").update("&nbsp;"));
		var valueInput = new Element("input", {
				"type": "text", "id": valueId, "name": "value", "class": "inputvalue", "size": 32, "value": value
			});
		...
		newDiv.insert(new Element("script").update(
			"new Ajax.Autocompleter('" + valueId + "', '" + valueId + "_choices', '" + URL.acValue+
			"', {'paramName': 'input', 'method': 'get', 'minChars': 1, 'frequency': 0.5, 'callback': autoCompleteCallBack});"
		));
		return newDiv;
}
...
function createAddTagIcon(nodeId, iconUrl, iconTitle, tags) {
	var elem = new Element("a", {
			"href": "#", "class": "ae-add-tag-icon",
			"onclick": "addTags('" + nodeId + "'," + Object.toJSON(tags) + ")"
		});
...
function createNewAmenityWizard(amenity) {
	...
	for (var key in wizardData) {
		elem.insert(new Element("a", {
				"href": "#", "class": "ae-create-amenity",
				"onclick": "addDefaultTags('"+nodeId+"',"+Object.toJSON(wizardData[key])+")"
			}).update(key));
	}
	return elem;
}
...
function createTitleDiv(amenity) {
	...
	if (amenity.nodeId != 0)
		div.insert(new Element("a", {
				"href": "https://www.openstreetmap.org/browse/node/" + amenity.nodeId, "target": "_blank"
			}).update("Id: " + amenity.nodeId));
	else
		div.insert("New Node");
...
function createEditBox(newDiv, amenity, feature) {
	newDiv.update(new Element("div", {
			"class": "ae-nodename"
		}).update("Node name: " + amenity.name));
	newDiv.insert(createTitleDiv(amenity));
	var editArea = new Element("div", {
			"class": "ae-editarea"
		});
	var keyValueTab = new Element("div", {
			"class": "ae-keyvaluetab", "id": "keyvaluetab_" + amenity.nodeId
		}).update(createKeyValueTable(amenity));
	if (amenity.nodeId == 0) {
		keyValueTab.hide();
		var newAmenityWizard = new Element("div", {
				"class": "ae-keyvaluetab", "id": "naw_" + amenity.nodeId
	...
	for (var x = 0; x < keyValueTemplates.length; x++) {
		...
		buttonDiv1.insert(createAddTagIcon(amenity.nodeId, contextPath + template.iconUrl, template.iconTitle, template.tags));
	}
	...
	var updateOsmButton = new Element("input", {
			"type":"button", "class": "ae-edit-button", "value": MSG.ebOsmButton, "title": MSG.ebOsmButtonTitle,
			"onclick": "updateKeyValueTable("+amenity.nodeId+")"
		});
	...
	var moveButton = new Element("input", {
			"type": "button", "class": "ae-edit-button", "value": MSG.ebMoveButton,
			"onclick": "moveAmenity(" + amenity.nodeId + ")"
		});
	if ((amenity.nodeId == 0) || AE.isMoving())
	...
	var deleteButton = new Element("input", {
			"type": "button", "class": "ae-edit-button", "value": MSG.ebDeleteButton,
			"onclick": "deleteAmenity(" + amenity.nodeId + ")"
		});
	if (amenity.nodeId == 0 || AE.isMoving())
		deleteButton.setAttribute("disabled", "disabled");
	buttonDiv.insert(deleteButton);
	buttonDiv.insert(new Element("input", {
			"type": "button", "class": "ae-edit-button", "value": MSG.ebSaveButton,
			"onclick": "saveAmenity(" + amenity.nodeId + ")"
		}));
	var closeButton = new Element("input", {
			"type": "button", "class": "ae-edit-button", value: MSG.ebCloseButton
		});
...
function saveFilterSettings() {
	...
	var filterChanged =
		checkboxesChanged(f.getInputs("checkbox", "show")) ||
		checkboxesChanged(f.getInputs("checkbox", "hide"));
	if (filterChanged) {
...
function loadSettingsFromCookie() {
	...
	if (cookies) {
		...
		for (var x = 0; x < cbs.length; x++) {
			cbs[x].checked = cbs[x].value == formData.show ||
				formData.show instanceof Array && formData.show.indexOf(cbs[x].value) != -1;
		}
		cbs = f.getInputs("checkbox", "hide");
		for (var x = 0; x < cbs.length; x++) {
			cbs[x].checked = cbs[x].value == formData.hide ||
				formData.hide instanceof Array && formData.hide.indexOf(cbs[x].value) != -1;
		}
...
function saveAmenity(nodeId) {
	...
	if (checkAccessRights()) {
		...
		if ((params["_nodeId"] == 0 || AE.isMoving()) &&
			(params.key.indexOf("highway") != -1 || params.key.indexOf("railway") != -1)) {
...
<body><div id="content" style="height:100%;overflow:hidden">
	<div style="position:absolute;top:0;left:80px;right:60px;z-index:750;background-color:#FFF;background-repeat:repeat-x;border:1px solid #000;border-top:none;-moz-border-radius:0 0 6px 6px;border-radius:0 0 6px 6px;padding:5px;height:32px">
		...
		<div style="position:relative;font-size:9px;bottom:-5px;left:0;font-weight:normal">
			Made by <a href="http://www.grundid-gmbh.de">GrundID GmbH</a>,
			...
			with <a href="https://code.google.com/p/google-maps-icons/">Maps icons collection (CC BY SA 3.0)</a>,
			Contact: as(a t)osmsurround(d o t)org, a href="https://www.openstreetmap.org/user/nitegate">OSM User: nitegate</a>,
			Source Code: <a href="https://github.com/grundid/amenity-editor">@GitHub</a>
		</div>
		<div style="position:absolute;top:5px;right:5px;width:780px;text-align:right">
			<c:if test="${startParameters.oauthTokenAvailable}"><span style="background-color:#ffe7cd;font-size:14px;padding:2px">OAuth OK!</span></c:if>
...
<c:forEach var="filter" items="<%= FilterManager.getInstance().getFiltersPositive() %>">
						<div style="float:left;width:170px">
							<input id="cb_${filter.name}" type="checkbox" class="filter-positive-cb" name="show" value="${filter.name}" <c:if test="${filter.defaultSelected}">checked="checked"</c:if>/>
							&nbsp;<label for="cb_${filter.name}"><spring:message code='filter.${filter.name}'/>&nbsp;<img width="12" height="12" src="<wt:ue>/img/${filter.icon}</wt:ue>"></label>
						</div>
</c:forEach>
						<div style="clear:both">
							<input type="button" class="ae-small-button" value="<spring:message code='button.select.all'/>" onclick="selectAll()"/>
							<input type="button" class="ae-small-button" value="<spring:message code='button.deselect.all'/>" onclick="deselectAll()"/>
							<input type="button" class="ae-small-button" value="<spring:message code='button.invert.selection'/>" onclick="invertSelection()"/>
						</div>
					</fieldset>
...
<c:forEach var="filter" items="<%= FilterManager.getInstance().getFiltersNegative() %>">
					...
					<fieldset>
						...
						<div style="position:relative;height:22px">
							...
							<span style="vertical-align:middle"><spring:message code='settings.lat'/>:&nbsp;</span><input type="text" name="lat" size="12" style="width:100px">
						</div>
					...
					<fieldset>
						...
						<input id="cb_saveincookie" type="checkbox" name="saveincookie" value="1"/>
						&nbsp;<label for="cb_saveincookie"><spring:message code='settings.saveincookie'/></label>
					</fieldset>
				</form>
				<p>
...
<%@ include file="includes/infobox.jspf" %>
			<div class="infobox" style="display:none;width:250px" id="feamenities">
				<spring:message code='info.fewamenities'/>
			</div>
			<div class="infobox" style="display:none;width:350px" id="moving">
			...
			<div class="infobox" style="width:170px;display:none" id="loading">
				<spring:message code="status.loading.data"/><br/>
				<img src="<wt:ue>/img/throbber.gif</wt:ue>">
			</div>
			<div class="infobox" style="display:none;width:170px" id="storing">
				<spring:message code='status.saving.data'/><br/>
				<img src="<wt:ue>/img/throbber.gif</wt:ue>">
			</div>
			<div class="infobox" style="display:none;width:400px" id="zoomStatus">
...
</div><noscript><spring:message code='no.javascript'/></noscript></body></html>
